...
/** V0.5: Last.fm similar/tag discovery + popularity metadata + MusicBrainz normalization. */
class ExternalDiscoveryClient(private val store: LocalStore) {
    private val musicBrainz = MusicBrainzClient()

    suspend fun discover(seeds: List<String>, genreLenses: List<String> = emptyList(), limitPerSeed: Int = 18): Result<ExternalDiscoveryResult> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            // Genre Lens is not just a ranking filter: add candidates from the selected genre's own pool.
            genreLenses.distinct().take(3).forEach { genre ->
                getTopArtistsByTag(key, genre, 14).forEach { c ->
                    val id = c.name.lowercase()
                    val existing = merged[id]
                    if (existing == null || c.match > existing.match) merged[id] = c
                }
            }
    ...
    private fun getTopArtistsByTag(apiKey: String, tag: String, limit: Int): List<Candidate> {
        ...
        return buildList {
            for (i in 0 until arr.length()) {
                ...
                val rankScore = (0.92f - i * 0.025f).coerceAtLeast(.55f)
    ...
    private fun lastFmGet(params: Map<String, String>): JSONObject {
        ...
        val connection = (URL("https://ws.audioscrobbler.com/2.0/?$query").openConnection() as HttpURLConnection).apply {
            ...
            setRequestProperty("User-Agent", "Metaranai-Android/0.5")
        }
