...
- 画面上は「あなたのメタルDNA」とDNA名を主役にし、8軸のDNA数値は読み取り専用表示を継続。
- DNA値とDNA名の自動学習・自動命名ロジック自体は維持。
- Android versionCode 20 / versionName 0.9.2。iOS Marketing Version 0.9.2 / Build 20。

## V0.9.1.1 — AUTO DNA CORRECTION

- DNA画面上部は「あなたのメタルDNA」と自動生成名だけに簡素化。学習回数・次回生成回数・手動編集不可の説明表示は廃止（自動学習ロジック自体は継続）。
